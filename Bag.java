...
public interface Bag {
    ...
    //capacity- default max value returned
    int capacity ();
    
    //isEmpty- returns false when there are no items in the bag, true otherwise
    boolean isEmpty ();
    
    //numOccur- occurances of item in the bag 
    int numOccur (Object item);
    
    //addItems- adds items to bag if there is space
    boolean addItems (Bag other);
    
    //equals- need to have same number of occurances
    boolean equals (Bag other);
    
    //unionWith- create and return an ArrayBag containing one occurrence of any item that is found in either the called object or the parameter other
